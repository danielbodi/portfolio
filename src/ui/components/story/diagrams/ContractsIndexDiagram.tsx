  ...
  return (
    ...
    >
      ...
      <div className="relative h-7" aria-hidden="true">
        <span className="absolute left-1/2 top-0 h-full w-px -translate-x-1/2 bg-purple-400/60 xl:h-1/2" />
        <span className="absolute left-1/4 right-1/4 top-1/2 hidden h-px bg-purple-400/50 xl:block" />
        <span className="absolute left-1/4 top-1/2 hidden h-1/2 w-px bg-purple-400/50 xl:block" />
        <span className="absolute right-1/4 top-1/2 hidden h-1/2 w-px bg-purple-400/50 xl:block" />
        <span className="absolute left-1/2 top-1/2 h-2.5 w-2.5 -translate-x-1/2 -translate-y-1/2 rounded-full border-2 border-purple-300 bg-slate-950 shadow-[0_0_0_4px_rgba(168,85,247,0.12)]" />
      </div>
        ...
        <article className="min-w-0 rounded-xl border border-slate-700/80 bg-slate-900/80 p-4">
          ...
          <ul className="mt-4 space-y-2">
            {knowledgeEntries.map((entry) => (
              ...
              >
                <code className="font-mono text-xs font-semibold text-purple-200">{entry.name}</code>
